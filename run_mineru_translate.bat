...
rem ========================================
rem PDF Translation Tool - Translation Process Script
rem Function: PDF -> Markdown -> Translate (no DOCX)
rem ========================================
...
echo [3/5] Running MinerU PDF conversion...
call mineru -p ./ -o ./
if errorlevel 1 (
...
echo [4/5] Translating markdown files...
python "%SCRIPT_DIR%translate_md.py"
if errorlevel 1 (
    echo [ERROR] Errors occurred during translation
    cd /d "%SCRIPT_DIR%"
    pause
    exit /b 1
)
...
echo [5/5] Copying files to output directory...
